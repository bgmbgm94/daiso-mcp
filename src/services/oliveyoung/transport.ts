...
export async function requestOliveyoung(
  ...
  if (relayUrl) {
    let relay: URL;
    try { relay = new URL(relayUrl); } catch {
      throw new Error('올리브영 릴레이 URL은 HTTPS 또는 로컬 HTTP 주소여야 합니다.');
    }
    if (relay.username || relay.password || relay.search || relay.hash ||
      (relay.protocol !== 'https:' && !(relay.protocol === 'http:' && ['127.0.0.1', 'localhost'].includes(relay.hostname)))) {
      throw new Error('올리브영 릴레이 URL은 HTTPS 또는 로컬 HTTP 주소여야 합니다.');
    }
  ...
  try {
    result = await fetchJson<OliveyoungApiResponse>(url, {
      method: 'POST', headers, body: JSON.stringify(body), timeout, retries: 0, expectedStatus: 200,
    });
  } catch (error) {
    ...
    throw new Error('올리브영 직접 요청 실패. 운영자는 OY_RELAY_URL과 OY_RELAY_TOKEN으로 브라우저 릴레이를 설정해주세요.', { cause: error });
  }
