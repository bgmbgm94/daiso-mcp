...
import type { DtryxShowtime } from '../types.js';
...
async function getRemainingSeats(args: GetRemainingSeatsArgs): Promise<McpToolResponse> {
  ...
  const cinemas = resolveDtryxCinemas(args);
  ...
  if (failedCinemas.length === cinemas.length) {
    return {
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify({ error: '모든 극장 조회에 실패했습니다.', failedCinemas }),
        },
      ],
    };
  }
  ...
  return {
    ...
    metadata: {
      ...
      inputSchema: {
        ...
        limit: dtryxLimitSchema.optional().default(50).describe('반환할 최대 회차 수 (기본값: 50)'),
