...
/** 극장 코드와 검색 필터를 함께 적용하며, 미등록 코드는 브랜드와 함께 허용합니다. */
export function resolveDtryxCinemas(args: {
  cinemaCode?: string;
  keyword?: string;
  region?: string;
  brandCode?: string;
}): DtryxCinema[] {
  let cinemas = [...DTRYX_CINEMAS];
  if (args.cinemaCode) {
  ...
  const keywordMatches = args.keyword ? findCinemasByKeyword(args.keyword) : undefined;
  const regionMatches = args.region ? findCinemasByRegion(args.region) : undefined;
  return cinemas.filter(
    (cinema) =>
      (!keywordMatches || keywordMatches.includes(cinema)) &&
      (!regionMatches || regionMatches.includes(cinema)) &&
      (!args.brandCode || cinema.brandCode === args.brandCode),
  );
}
