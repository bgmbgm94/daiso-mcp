...
function jsonResponse(body: unknown) {
  return new Response(
    JSON.stringify({ RetCode: 'success', ...(body as Record<string, unknown>) }),
    {
      status: 200,
...
describe('핸들러 분기', () => {
  ...
  it('카탈로그에 없는 극장이 실패하면 코드로 표기한다', async () => {
    ...
    )) as unknown as { status: number; data: { error: { message: string } } };

    expect(result.status).toBe(503);
    expect(result.data.error.message).toContain('000999');
  });
